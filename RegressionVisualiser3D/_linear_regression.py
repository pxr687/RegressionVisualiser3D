...
def _linear_regression(intercept=None,
    ...
    # set default intercept, slopes, error sd and axes limits if none are 
    ...
    intercept, predictor1_slope, predictor2_slope, error_sd,  = _get_legal_default_param_combos_for_current_model(
                                                                ...
                                                                predictor2_slope,
                                                                error_sd, 
                                                                "linear_legal_default_params.csv")

    # =============================================
    # GENERATING THE POPULATION DATA

    # generate the population x and y data points
    ...
    # generate the population z datapoints, based on the slopes and intercept
    ...
    # try to fit the regression model to the population data, warn the user
    # in the case of computation errors
    try:
        mod_pop = _sm.OLS(population_df[outcome_variable_name], _sm.add_constant(population_df[[
    ...
    pop_z = mod_pop_intercept + mod_pop_predictor1_slope*x + \
        mod_pop_predictor2_slope*y + mod_pop_interaction_slope * x*y
    
    # plot the population regression surface and data
    fig = _plt.figure(figsize=plot_size)
    ax1 = fig.add_subplot(121, projection="3d")
    ax1.plot_wireframe(x, y, pop_z, color="red",
                        label="population linear regression surface", alpha=surface_alpha)
    ax1.scatter(pop_data_x, pop_data_y, pop_data_z, color="darkred",
    ...
    ax2.plot_wireframe(x, y, sample_z, color="blue",
    ...
    ax2.scatter(sample_df[predictor1_name], sample_df[predictor2_name],
                sample_df[outcome_variable_name], color="darkblue", label="sample data", alpha=data_alpha)
